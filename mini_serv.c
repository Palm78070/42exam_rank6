...
int getId(int clientSockets[], int socketId, int next_id)
{
...
int main(int argc, char **argv)
{
	...
	// char discon_buffer[BUFFER_SIZE];
		...
		for (int socketId = 0; socketId <= maxSocket; socketId++)
		{
			if (FD_ISSET(socketId, &readySockets))
			{
				...
				else
				{
					...
					char broadcast_buff[BUFFER_SIZE];
					// int bytesRead = recv(socketId, buffer, sizeof(buffer) - 1, 0);
					int bytesRead = recv(socketId, broadcast_buff, sizeof(broadcast_buff) - 1, 0); //
					broadcast_buff[bytesRead] = '\0';
					//  Receive data from the client

					if (bytesRead <= 0)
					{
						int id = getId(clientSockets, socketId, next_id);
						// Client disconnected
					...
					else
					{
						...
						strcpy(tmp_buff, broadcast_buff);
						tmp_buff[strlen(broadcast_buff)] = '\0';
						sprintf(broadcast_buff, "client %d: %s", id, tmp_buff);
						// sprintf(broadcast_buff, "client %d: %s", socketId, tmp_buff);
						...
						for (int i = 0; i < next_id; i++)
						{
							if (clientSockets[i] != socketId)
							{
								...
								send(clientSockets[i], broadcast_buff, strlen(broadcast_buff), 0);
							}
						}
					}
				}
